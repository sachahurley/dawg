# DAWG MCP on every app (your Mac)
...
|-----|--------------------|---------------------------|
| **Claude Code** (terminal, or inside Cursor/VS Code) | **User-scoped** MCP — available in **every folder** | Nothing if `claude mcp add-json … --scope user` already ran on this machine. Run **`/mcp`** in a session and confirm **dawg** lists. |
...
| **Cursor** | Not stored in this repo (Cursor has its own MCP UI) | **Cursor → Settings → MCP** → add the same `command` / `args` / `env` as `templates/mcp-dawg.json` once (applies to all workspaces). |
| **VS Code + Claude Code** | Same as **Claude Code** row | If you use the **Claude Code** extension and it uses your user MCP config, **dawg** follows you. Plain VS Code without Claude Code has **no** built-in MCP for this server. |
...
```bash
claude mcp add-json dawg \
  '{"type":"stdio","command":"node","args":["/Users/sachahurley/Projects/dawg/rag-server/dist/index.js"],"env":{"KNOWLEDGE_BASE_PATH":"/Users/sachahurley/Projects/dawg","OLLAMA_HOST":"http://localhost:11434","CHROMA_HOST":"localhost","CHROMA_PORT":"8000"}}' \
  --scope user
```
...
2. Run: `claude mcp add-from-claude-desktop --scope user` and select **dawg**.

## Cursor (still manual once)

1. Open **Cursor Settings** (Cmd + ,) → search **MCP** or **Model Context Protocol**.
2. Add a server named `dawg` with the contents of **`templates/mcp-dawg.json`** (same paths as your machine).
