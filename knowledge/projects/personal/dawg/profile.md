...
- **`npm run doctor` is the check that matters after a reboot or a move.** It tests recoverability, not just status: LaunchAgent flags and working directory, both services, the index against disk, and whether `chroma` will actually execute. It exits non-zero on failure, so it is usable from a cron or a hook. Verified against the real breakage: reintroducing the bad shebang makes it fail with the right diagnosis.
